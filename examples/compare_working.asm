; Compares a and b following C's qsort convention---negative if a<b, zero if
; equal, positive if a>b---without ever overflowing, then prints the result.
; With a = -20000 and b = 20000 the program prints -20000.

.ORIG x3000
cint  LD R1,a
      BRn aNeg           ; a>=0 falls through
      LD R2,b
      BRn retA
      BR cmp
aNeg  LD R2,b
      BRn cmp
; a and b have different signs: return a
retA  ST R1,r
      BR leave
; a and b have the same sign: return a-b, which cannot overflow
cmp   NOT R2,R2
      ADD R2,R2,#1
      ADD R2,R1,R2
      ST R2,r

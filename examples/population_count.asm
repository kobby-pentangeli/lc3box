; Counts the set bits (population count) of R1 and prints the total as decimal.
; With R1 = xB1E5 (nine bits set) the program prints 9.

.ORIG x3000
pop     LD  R1, VALUE
        AND R0, R0, #0
        ADD R1, R1, #0        ; test the most significant bit
        BRzp skipf
        ADD R0, R0, #1
skipf   AND R2, R2, #0
        ADD R2, R2, #15       ; fifteen bits remain
loop    ADD R1, R1, R1        ; shift the next bit into the sign position
        BRzp skip
        ADD R0, R0, #1
skip    ADD R2, R2, #-1
        BRp loop

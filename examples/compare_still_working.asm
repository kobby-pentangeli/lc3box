; Compares a and b by folding a's sign bit into b---a branch-light alternative
; to compare_working.asm, same qsort convention---then prints the result.
; With a = -20000 and b = 20000 the program prints -20000.

.ORIG x3000
cint  LD R3,b15
      LD R1,a
      LD R2,b
      AND R3,R3,R1         ; R3 = the sign bit of a
      ADD R3,R3,R2         ; negative only when a and b have different signs
      BRzp cmp
; different signs: return a
retA  ST R1,r
      BR leave
; same sign: return a-b, which cannot overflow
cmp   NOT R2,R2
      ADD R2,R2,#1
      ADD R2,R1,R2
      ST R2,r
